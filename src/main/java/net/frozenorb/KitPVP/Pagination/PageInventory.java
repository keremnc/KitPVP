...
public abstract class PageInventory extends ClickInventory {
	...
	public PageInventory(Player player, boolean dynamicInventory) {
		...
		user = player;
		user.setMetadata(getClass().getSimpleName(), new FixedMetadataValue(KitPVP.get(), this));
	}
	...
	public ItemStack getBackPage() {
		if (backAPage == null) {
			...
			backAPage = Core.get().generateItem(item.getType(), item.getDurability(), ChatColor.RED + "Back", new String[] { ChatColor.BLUE + "Click this to move back" });
	...
	public ItemStack getForwardsPage() {
		if (forwardsAPage == null) {
			ItemStack item = new ItemStack(Material.SUGAR_CANE_BLOCK);
			forwardsAPage = Core.get().generateItem(item.getType(), item.getDurability(), ChatColor.RED + "Forward", new String[] { ChatColor.BLUE + "Click this to move forward" });
	...
	public void onClose(InventoryCloseEvent event) {
		if (listenForClose && event.getPlayer() == user) {
			...
			if (user.hasMetadata(getClass().getSimpleName()) && user.getMetadata(getClass().getSimpleName()).get(0).value() == this)
				user.removeMetadata(getClass().getSimpleName(), KitPVP.get());
		}
	}
